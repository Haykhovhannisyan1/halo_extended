...
impl<F: WithSmallOrderMulGroup<3>> EvaluationDomain<F> {
    ...
    pub fn new(j: u32, k: u32) -> Self {
        ...
        let mut omega_powers = vec![];
        for i in 0..n {
            let mut tmp = vec![];
            for j in 0..n {
                    tmp.push(omega.pow_vartime([(j*i) as u64, 0, 0, 0]));
            }
            omega_powers.push(tmp.clone());
        }
    ...
    /// Get $\omega$ powers for constructing b vector
    pub fn get_omega_powers_i(&self, index: u64) -> Vec<F> {
        self.omega_powers[index as usize].clone()
    }
