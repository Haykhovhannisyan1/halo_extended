...
>(
    ...
    let advice: Vec<AdviceSingle<C>> = circuits
        ...
        .map(|(circuit, instances)| -> Result<AdviceSingle<C>, Error> {
            ...
            // //////   ZKFFT
            // let advice_witness = advice_polys.clone();
            // let n = advice_witness[0].num_coeffs();
            // let a: Vec<C::Scalar> = advice_witness[0].iter().copied().collect();
            // let mut b = Vec::with_capacity(n);
            // let mut final_commit = advice_commitments[0];
            // // let omega = domain.get_omega();
            // // println!("unusable_rows_start: {:?}", unusable_rows_start);
            // let start_time = Instant::now();
            // for i in 0..unusable_rows_start {
            //     b.push(domain.get_omega_powers_i(i as u64));
            //     final_commit = final_commit
            //         .add((params.gen_g[i] * advice[0][i]).into())
            //         .into();
            // }
            // let tmp = vec![C::Scalar::ZERO; n];
            // for _ in unusable_rows_start..n {
            //     b.push(tmp.clone());
            // }
            // let end_time = Instant::now();
            // let elapsed_time = end_time.duration_since(start_time);
            // // println!("b time prover: {:?}ms", elapsed_time.as_millis());

            // let alpha = advice_blinds[0].0; //C::Scalar::random(&mut rng);
            // let wip_witness = optimized_wip::WipWitness::<C> {
            //     a: a.clone(),
            //     b: b.clone(),
            //     alpha: alpha.clone(),
            // };
            // transcript.write_point(final_commit)?;
            // // let end_time = Instant::now();
            // // let elapsed_time = end_time.duration_since(start_timef);
            // // println!("final commit time prover: {:?}ms", elapsed_time.as_millis());
            // optimized_wip::prove(
            //     transcript,
            //     wip_witness,
            //     params.g.clone(),
            //     params.gen_g.clone(),
            //     params.w,
            //     optimized_wip::P::Point(final_commit),
            // );
